...
public class TaxCalculator {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(" whats your annual salary? ");
        int salary = in.nextInt();
        System.out.println(" Are you married ?  (Y/N) ");
        boolean status = in.nextBoolean();

        double taxToBePaid = calculateTax(salary, status);// u can change for true
        printMyFederalTaxInfo(taxToBePaid);

        double taxForSingle = calculateTaxForSingle(salary);
        printMyFederalTaxInfo(taxForSingle);

        double taxForMarried=calculateTaxForMarried(salary);
        printMyFederalTaxInfo(taxForMarried);
